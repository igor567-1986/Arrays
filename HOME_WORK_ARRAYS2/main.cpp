...
void main()
{
	...
	cout << "Ќахождение повтор€ющихс€ элементов массива." << endl;
	int  arr[n];
	srand(time(NULL));
	for (int i = 0; i < n; i++)
	{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = i + 1; j < n; j++)
		{
         ...
         if(counter==1) cout<< arr[i]<< " повтор€етс€ в массиве "<< counter << "раз"<<endl;
		 if (counter > 3)
		 {
			 cout << arr[i] << " повтор€етс€ в массиве " << counter << "раза" << endl;
			 a = arr[i];
		 }
        counter = 1;
	}
